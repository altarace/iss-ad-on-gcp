...
Param(
	...
	$Zone,
	[Parameter()][Boolean]
	$UseMinimalResources = $True,   # designed for free tier
	[Parameter()][Boolean]
...
# store parameters in gcs encrypting the secret
Write-Host "Saving parameters..."
$Temp = New-TemporaryFile
...
While ( $(gcloud deployment-manager operations list --project $Project --filter "TARGET:$Prefix-$Suffix AND TYPE:insert" --format 'value(STATUS)') -ne 'DONE' ) {
		...
		--project $Project `
                --filter "TARGET:$Prefix-$Suffix AND TYPE:insert AND STATUS:RUNNING" `
                --format 'value(NAME)' ) `
		--project $Project
...
$Jobs = gcloud compute instances list --project $Project --format "value(name)" | ForEach-Object {
